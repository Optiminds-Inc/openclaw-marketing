...
9. 查看交互：`agent-browser snapshot -i`
10. 下一步：`agent-browser click @e15`
11. 查看交互：`agent-browser snapshot -i`
12. 在描述框输入简介：`agent-browser type ".ProseMirror" "{简介内容}"`
13. 添加话题：`agent-browser type ".ProseMirror" " #{话题}"`，然后 `agent-browser press "Enter"` 确认
14. 暂存草稿离开：`agent-browser click @e13`
